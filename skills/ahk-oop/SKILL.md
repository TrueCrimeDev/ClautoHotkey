...
1. **Read** `${CLAUDE_PLUGIN_ROOT}/Modules/Module_Classes.md` — Class design, inheritance, constructors, destructors, static members
2. **Read** `${CLAUDE_PLUGIN_ROOT}/Modules/Module_Objects.md` — Object hierarchy, property descriptors, method binding
3. **Read** `${CLAUDE_PLUGIN_ROOT}/Modules/Module_Arrays.md` — 1-based indexing, Array.ahk library (Join/Filter/Map/Reduce/Sort)
4. **Read** `${CLAUDE_PLUGIN_ROOT}/Modules/Module_DynamicProperties.md` — Dynamic properties via __Get/__Set, closures, computed properties
5. **Read** `${CLAUDE_PLUGIN_ROOT}/Modules/Module_ClassPrototyping.md` — Prototyping workflows, runtime class creation
