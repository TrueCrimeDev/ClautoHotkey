...
2. Load the full reference — capability matrix + a portable fallback for every gated feature —
   from `${CLAUDE_PLUGIN_ROOT}/Modules/Module_Versions.md`. It maps each feature to the builds that support it.
...
read-only class base, `GuiCtrlFromHwnd` no-value), read `${CLAUDE_PLUGIN_ROOT}/Modules/Module_Versions.md`.
