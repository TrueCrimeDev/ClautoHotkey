...
class ReactiveRuntime {
    ...
    static EndBatch() {
        if this.BatchDepth <= 0
            throw Error("EndBatch called without BeginBatch.")
...
class ReactiveNode {
    ...
    _Notify(value, oldValue := unset) {
        snapshot := []
        for subscription, callback in this.Subscribers
...
class Signal extends ReactiveNode {
    ...
    Update(transform) {
        return this.Set(transform.Call(this._Value))
    }
