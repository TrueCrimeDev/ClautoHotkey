...
class ResponsiveListManager {
  ...
  InitializeState() {
    this.state := Map(
      "currentFile", A_WorkingDir . "\__List.json",
  ...
  CreateHeaderSection() {
    ...
    this.controls["saveBtn"] := this.gui.AddButton(
      Format("x{} y{} w{} h{}",
        buttonX,
        currentY,
        ResponsiveListManager.ICON_BTN_SIZE,
  ...
  CreateMainSection() {
    ...
    mainContentHeight := ResponsiveListManager.MIN_GUI_HEIGHT - mainY - ResponsiveListManager.BUTTON_HEIGHT - (ResponsiveListManager.MARGIN * 2) - ResponsiveListManager.SECTION_SPACING
  ...
  CreateButtonSection() {
    margin := ResponsiveListManager.MARGIN
    buttonY := ResponsiveListManager.MIN_GUI_HEIGHT - margin - ResponsiveListManager.BUTTON_HEIGHT

    this.layout["buttonY"] := buttonY
    ...
    for btn in iconButtons {
      this.controls[btn[1]] := this.gui.AddButton(
        Format("x{} y{} w{} h{}",
          currentX,
          buttonY,
          ResponsiveListManager.ICON_BTN_SIZE,
      ...
      currentX += ResponsiveListManager.ICON_BTN_SIZE + ResponsiveListManager.BUTTON_GAP
    }
  }
  ...
  ApplyDarkTheme() {
    ...
    mainContentHeight := ResponsiveListManager.MIN_GUI_HEIGHT - this.layout["mainY"] - ResponsiveListManager.BUTTON_HEIGHT - (ResponsiveListManager.MARGIN * 2) - ResponsiveListManager.SECTION_SPACING
  ...
  ApplyControlTheming() {
    ...
    this.controls["reloadBtn"].SetFont("s16 Bold")
    this.controls["duplicateBtn"].SetFont("s14 Bold")
    this.controls["saveBtn"].SetFont("s12")
    ...
    this.controls["saveBtn"].ToolTip := "Save File (Ctrl+S)"
  ...
  SetupControlEvents() {
    ...
    this.controls["saveBtn"].OnEvent("Click", this.OnSaveFile.Bind(this))
  ...
  OnResize(guiObj, minMax, width, height) {
    ...
    buttonRowY := height - margin - ResponsiveListManager.BUTTON_HEIGHT
    mainContentHeight := buttonRowY - this.layout["mainY"] - ResponsiveListManager.SECTION_SPACING
    ...
    iconX := margin
    this.controls["addBtn"].Move(iconX, buttonRowY)
    iconX += ResponsiveListManager.ICON_BTN_SIZE + ResponsiveListManager.BUTTON_GAP
    this.controls["reloadBtn"].Move(iconX, buttonRowY)
  }
    ...
    if _Dark.ControlMaps["HeaderColors"].Has(HWND) && HC := _Dark.ControlMaps["HeaderColors"][HWND] {
      ...
      if (Code = NM_CUSTOMDRAW) {
        ...
        if (DrawStage = CDDS_ITEMPREPAINT) {
          ...
          ; Always paint the background for all columns
        ...
        if (DrawStage = CDDS_PREPAINT) {
          ; Paint the entire header background first
          HDC := NumGet(lParam + OHDC, "Ptr")
          dcBrush := DllCall("GetStockObject", "UInt", DC_BRUSH, "ptr")
          DllCall("SetDCBrushColor", "Ptr", HDC, "UInt", HC["Bkg"])
          DllCall("FillRect", "Ptr", HDC, "Ptr", lParam + ORect, "Ptr", dcBrush)
