...
class ResponsiveListManager {
  ...
  CreateHeaderSection() {
    ...
    ; Row 1: ComboBox - Force exact 150px width
    ...
    currentY += ResponsiveListManager.CONTROL_HEIGHT + 5

    ; Row 2: Buttons and Search with mathematical positioning
    ; Calculate positions for second row
    addBtnX := margin
    ...
    ; Calculate search width to fit exactly within left panel boundary
    ...
    currentY += ResponsiveListManager.CONTROL_HEIGHT
  ...
  ApplyComboBoxDarkTheme() {
    ...
    ; Force ComboBox to exact 150px width - no Move() needed as it's set in creation
    ...
    DllCall("user32\SendMessage", "Ptr", this.controls["sectionSelector"].hWnd, "UInt", CB_SETDROPPEDWIDTH, "Ptr", ResponsiveListManager.COMBO_WIDTH, "Ptr", 0)
  ...
  OnResize(guiObj, minMax, width, height) {
    ...
    ; Recalculate search box position and width using same mathematical positioning
    duplicateBtnX := margin + ResponsiveListManager.ICON_BTN_SIZE + gap
    searchX := duplicateBtnX + ResponsiveListManager.ICON_BTN_SIZE + gap
    searchWidth := leftPanelBoundary - searchX
